...
def ls(expr = 'wrfout_d03_2018-03*'):
    return glob(expr)
...
ncfile = '/home/opti3040a/Documentos/WRF_marzo/wrfout/wrfout_mar.2018/wrfout_d03_2018-03-03_13:00:00'     # el programa debe estar en la misma carpeta de las extensiones .nc
...
alt = rootgroup.variables["HGT"][:]     # read height variables
...
#               looking for latitude near to Weather Station from Qollana
#               *********************************************************
idx_lat = (y[0,:,1]>=-17.64)*(y[0,:,1]<=-17.60) # comparison of domains in each grid of 1000m2
...
f = MFDataset("wrfout_d03_2018-03*")
...
#   +---------->
#               U
# Transformando a Posicion de la tierra

ue = u10m * cosalpha[[idxlat,idxlon]] - v10m * sinalpha[[idxlat,idxlon]]
ve = v10m * cosalpha[[idxlat,idxlon]] + u10m * sinalpha[[idxlat,idxlon]]
# print(cosalpha[[idxlat,idxlon]])
# ue = u10m
# ve = v10m

R = np.sqrt((ue*ue) + (ve*ve))
print("\n Resultant of the velocity of the wind in m/s")
print(R)                                # Print the resultant

#  generating the direction of the resultant
tetha = np.arctan((ve/ue))          # tetha in radians
...
df2.to_csv(path_or_buf="df_marzoCorr.csv")       # save dataframe dataframe.to_csv()
